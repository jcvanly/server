#include <server_hardware_model.hpp>

namespace Nos3
{
    REGISTER_HARDWARE_MODEL(ServerHardwareModel,"SERVER");
    ...
    _enabled(SERVER_SIM_SUCCESS), _count(0), _config(0), _status(0)
    {
        ...
        sim_logger->info("ServerHardwareModel::ServerHardwareModel:  NOS Engine connection string: %s.", connection_string.c_str());

        /* Get a data provider */
        std::string dp_name = config.get("simulator.hardware-model.data-provider.type", "SERVER_PROVIDER");
        _server_dp = SimDataProviderFactory::Instance().Create(dp_name, config);
        sim_logger->info("ServerHardwareModel::ServerHardwareModel:  Data provider %s created.", dp_name.c_str());
        ...
        _uart_connection.reset(new NosEngine::Uart::Uart(_hub, config.get("simulator.name", "server_sim"), connection_string, bus_name));
        _uart_connection->open(node_port);
        sim_logger->info("ServerHardwareModel::ServerHardwareModel:  Now on UART bus name %s, port %d.", bus_name.c_str(), node_port);
    
        /* Configure protocol callback */
        _uart_connection->set_read_callback(std::bind(&ServerHardwareModel::uart_read_callback, this, std::placeholders::_1, std::placeholders::_2));
        ...
        sim_logger->info("ServerHardwareModel::ServerHardwareModel:  Now on time bus named %s.", time_bus_name.c_str());

        /* Construction complete */
        sim_logger->info("ServerHardwareModel::ServerHardwareModel:  Construction complete.");
    }


    ServerHardwareModel::~ServerHardwareModel(void)
    {        
    ...
    void ServerHardwareModel::command_callback(NosEngine::Common::Message msg)
    {
        /* Get the data out of the message */
        NosEngine::Common::DataBufferOverlay dbf(const_cast<NosEngine::Utility::Buffer&>(msg.buffer));
        sim_logger->info("ServerHardwareModel::command_callback:  Received command: %s.", dbf.data);

        /* Do something with the data */
        std::string command = dbf.data;
        std::string response = "ServerHardwareModel::command_callback:  INVALID COMMAND! (Try HELP)";
        boost::to_upper(command);
        if (command.compare("HELP") == 0) 
        {
            response = "ServerHardwareModel::command_callback: Valid commands are HELP, ENABLE, DISABLE, STATUS=X, or STOP";
        }
        else if (command.compare(0,6,"ENABLE") == 0) 
        {
            _enabled = SERVER_SIM_SUCCESS;
            response = "ServerHardwareModel::command_callback:  Enabled\n";
        }
        else if (command.compare(0,7,"DISABLE") == 0) 
        {
            _enabled = SERVER_SIM_ERROR;
            _count = 0;
            _config = 0;
            _status = 0;
            response = "ServerHardwareModel::command_callback:  Disabled";
        }
        else if (command.substr(0,7).compare("STATUS=") == 0)
        {
            try
            {
                ...
                response = "ServerHardwareModel::command_callback:  Status set";
            }
            catch (...)
            {
                response = "ServerHardwareModel::command_callback:  Status invalid";
            }            
        }
        else if (command.compare(0,4,"STOP") == 0) 
        {
            _keep_running = false;
            response = "ServerHardwareModel::command_callback:  Stopping";
        }
        /* TODO: Add anything additional commands here */

        /* Send a reply */
        sim_logger->info("ServerHardwareModel::command_callback:  Sending reply: %s", response.c_str());
        _command_node->send_reply_message_async(msg, response.size(), response.c_str());
    }


    /* Custom function to prepare the Server HK telemetry */
    void ServerHardwareModel::create_server_hk(std::vector<uint8_t>& out_data)
    {
        /* Prepare data size */
    ...
    /* Protocol callback */
    void ServerHardwareModel::uart_read_callback(const uint8_t *buf, size_t len)
    {
        std::vector<uint8_t> out_data; 
        std::uint8_t valid = SERVER_SIM_SUCCESS;
        
        /* Retrieve data and log in man readable format */
        std::vector<uint8_t> in_data(buf, buf + len);
        sim_logger->debug("ServerHardwareModel::uart_read_callback:  REQUEST %s",
            SimIHardwareModel::uint8_vector_to_hex_string(in_data).c_str());

        /* Check simulator is enabled */
        if (_enabled != SERVER_SIM_SUCCESS)
        {
            sim_logger->debug("ServerHardwareModel::uart_read_callback:  Server sim disabled!");
        ...
        else
        {
            ...
            if (in_data.size() != 9)
            {
                sim_logger->debug("ServerHardwareModel::uart_read_callback:  Invalid command size of %ld received!", in_data.size());
                valid = SERVER_SIM_ERROR;
            }
            else
            {
                ...
                {
                    sim_logger->debug("ServerHardwareModel::uart_read_callback:  Header incorrect!");
                    valid = SERVER_SIM_ERROR;
                }
                else
                {
                    ...
                    {
                        sim_logger->debug("ServerHardwareModel::uart_read_callback:  Trailer incorrect!");
                        valid = SERVER_SIM_ERROR;
                    }
                    else
                    {
            ...
            {   
                ...
                case 1:
                    ...
                    case 2:
                        /* Request data */
                        sim_logger->debug("ServerHardwareModel::uart_read_callback:  Send data command received!");
                        create_server_data(out_data);
                        break;

                    case 3:
                        /* Configuration */
                        sim_logger->debug("ServerHardwareModel::uart_read_callback:  Configuration command received!");
                    ...
                    default:
                        /* Unused command code */
                        valid = SERVER_SIM_ERROR;
                        sim_logger->debug("ServerHardwareModel::uart_read_callback:  Unused command %d received!", in_data[2]);
                        break;
                }
            }
        }
        ...
        if (valid == SERVER_SIM_SUCCESS)
        {
            ...
            if (out_data.size() > 0)
            {
                sim_logger->debug("ServerHardwareModel::uart_read_callback:  REPLY %s",
