...
** Global Variables
*/
uart_info_t ServerUart;
SERVER_Device_HK_tlm_t ServerHK;
SERVER_Device_Data_tlm_t ServerData;
...
int process_command(int cc, int num_tokens, char tokens[MAX_INPUT_TOKENS][MAX_INPUT_TOKEN_SIZE])
{
    ...
    {	
        ...
        case CMD_NOOP:
            ...
            {
                status = SERVER_CommandDevice(&ServerUart, SERVER_DEVICE_NOOP_CMD, 0);
        ...
        case CMD_HK:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = SERVER_RequestHK(&ServerUart, &ServerHK);
                if (status == OS_SUCCESS)
                {
                    OS_printf("SERVER_RequestHK command success\n");
                }
                else
                {
                    OS_printf("SERVER_RequestHK command failed!\n");
                }
            }
            break;
        ...
        case CMD_SERVER:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = SERVER_RequestData(&ServerUart, &ServerData);
                if (status == OS_SUCCESS)
                {
                    OS_printf("SERVER_RequestData command success\n");
                }
                else
                {
                    OS_printf("SERVER_RequestData command failed!\n");
                }
            }
            break;
        ...
        case CMD_CFG:
            if (check_number_arguments(num_tokens, 1) == OS_SUCCESS)
            {
                ...
                status = SERVER_CommandDevice(&ServerUart, SERVER_DEVICE_CFG_CMD, config);
                if (status == OS_SUCCESS)
                {
...
int main(int argc, char *argv[]) 
{
    ...
    ServerUart.deviceString = SERVER_CFG_STRING;
    ServerUart.handle = SERVER_CFG_HANDLE;
    ServerUart.isOpen = PORT_CLOSED;
    ServerUart.baud = SERVER_CFG_BAUDRATE_HZ;
    status = uart_init_port(&ServerUart);
    if (status == OS_SUCCESS)
    {
        printf("UART device %s configured with baudrate %d \n", ServerUart.deviceString, ServerUart.baud);
    }
    else
    {
        printf("UART device %s failed to initialize! \n", ServerUart.deviceString);
        run_status = OS_ERROR;
    }
    ...
    // Close the device 
    uart_close_port(&ServerUart);

    #ifdef _NOS_ENGINE_LINK_
        nos_destroy_link();
    #endif

    OS_printf("Cleanly exiting server application...\n\n"); 
    return 1;
}
